...
function normalizeProductType(value) {
  ...
  if (typeof value === "object" && value !== null) {
    if (
      typeof value.platformCategoryPath === "string" &&
      value.platformCategoryPath.trim()
    ) {
      return value.platformCategoryPath.trim();
    }

    if (
      typeof value.baseCategory === "string" &&
      value.baseCategory.trim()
    ) {
      return value.baseCategory.trim();
    }

    if (
      typeof value.regionalCategory === "string" &&
      value.regionalCategory.trim()
    ) {
      return value.regionalCategory.trim();
    }
